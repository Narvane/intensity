...
class BoxIntegrationTest extends AbstractMockMvcIntegrationTest {
	...
	void listGroupsRequiresAuthentication() throws Exception {
		mockMvc.perform(get("/v1/groups")).andExpect(status().isUnauthorized());
	}
	...
	void experiencesSessionCanListBoxesAfterMembership() throws Exception {
		...
		mockMvc.perform(get("/v1/groups").header("Authorization", "Bearer " + token))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$", hasSize(1)));
	...
	void experiencesSessionCannotDeleteBox() throws Exception {
		...
		mockMvc.perform(delete("/v1/boxes/{boxId}", boxId).header("Authorization", "Bearer " + token))
	...
	void deleteBoxCascadesExperiences() throws Exception {
		...
		mockMvc.perform(delete("/v1/boxes/{boxId}", boxId)
						.header("Authorization", "Bearer " + experienceBoxToken))
	...
	private void ensureBoxCreated() throws Exception {
		...
		MvcResult result = mockMvc.perform(post("/v1/boxes")
						.header("Authorization", "Bearer " + experienceBoxToken)
						.contentType(MediaType.APPLICATION_JSON)
						.content("""
								{
