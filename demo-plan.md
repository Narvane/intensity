...
| Item | Valor |
|------|--------|
| URL | `https://demo-intensity.<domínio>` (SPA React) |
| API | `https://demo-intensity-api.<domínio>` |
...
Internet :443
  ...
  Caddy (pode ser o mesmo container com hosts extras, ou Compose demo)
    ├── demo-intensity.<domínio>      → arquivos estáticos do Vite (client dist)
    └── demo-intensity-api.<domínio>  → api:8080 (SPRING_PROFILES_ACTIVE=demo)
...
| CORS abrir o demo no domínio errado | Em `application-demo.yml`, permitir **só** `https://demo-intensity.<domínio>` (+ localhost p/ debug) |
...
| Risco | Mitigação |
|-------|-----------|
| Invite do demo gerar link de **prod** | Build demo com `VITE_INVITE_BASE_URL=https://demo-intensity.<domínio>/join` |
| App de loja abrir link demo e bater na API **prod** | Host diferente (`demo-intensity.` ≠ `app.`); associação Universal/App Links **não** inclui o host demo |
...
5. **Não** registrar `demo-intensity.<domínio>` nos well-known de produção / manifests nativos.
...
2. Caddy (ou nginx alpine) serve `dist/` em `demo-intensity.<domínio>`.
...
- [ ] Domínios DNS: `demo-intensity.` e `demo-intensity-api.` (ou path único atrás do mesmo host — preferir dois subdomínios, espelha prod).
- [ ] Confirmar que a VPS aguenta o segundo Postgres.  
- [ ] Textos finais Leo/Maya/Nico + senha `demo1234`.

### Fase 1 — API demo profile + seed

- [x] `application-demo.yml` + CORS.  
- [x] Seed runner idempotente (3 participants, 2 groups, 5 boxes, ~18 experiences, allowlist).  
- [x] Teste local: `DemoSeedIntegrationTest` (+ `SPRING_PROFILES_ACTIVE=demo` contra `intensity_demo`).  
- [x] Script `api/scripts/reset-demo-db.sh` + init `intensity_demo` no `api/docker-compose.yml`.

### Fase 2 — Compose + Caddy demo na VPS

- [x] `docker-compose.demo.yml` + `.env.demo.example`.  
- [x] `deploy-demo.sh`.  
- [x] TLS via Caddy (snippet + shared network `intensity`).  
- [x] Health check `demo-api` / static placeholder `demo-web`.  
- [x] Cron de reset (`reset-demo.sh` + `cron-reset-demo.sh`).

### Fase 3 — Client modo demo

- [x] `.env.demo` + build mode (`npm run build:demo`).  
- [x] Banner `VITE_DEMO`.  
- [x] Publicar `dist` (`deploy/publish-demo-client.sh` → `demo-web/`).  
- [x] `/join` via nginx `try_files` + `VITE_INVITE_BASE_URL` demo.  
- [x] Atalhos de login (Leo / Maya / Nico · casal · trio).

### Fase 4 — DX e portfolio

- [x] Seção no @ref:readme com URL + credenciais + “reset diário”.  
- [x] Atualizar @ref:en-platforms (e pt-br/it) mencionando ambiente **demo** sem chamar de staging de release.  
- [ ] ~~(Opcional) CI: job `deploy-demo`~~ — deferido; atualizar demo com `./deploy-demo.sh` / `./publish-demo-client.sh` após o webhook de prod.  
- [ ] ~~(Opcional) screenshot / GIF no README~~ — fora desta entrega.
...
- [ ] Abrir `https://demo-intensity.<domínio>` sem instalar nada.
...
- [ ] Invite gerado no demo aponta para `https://demo-intensity.<domínio>/join?t=…` e **nunca** para o host de prod.
