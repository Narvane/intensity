...
- **Modifica esperienza** — nessun flusso di modifica osservato; solo creare, alternare rivelazione ed eliminare (esperienze proprie).
- **Gestione box** — nessuna rinomina, modifica o eliminazione box osservata.
- **Gestione gruppo** — i gruppi sono impliciti; nessuna interfaccia creazione o denominazione manuale gruppo.
- **Cattura conseguenza** — la guida rapida fa riferimento alle conseguenze; nessuna schermata di inserimento trovata.
- **Notifiche** — nessun sistema push o notifica in-app osservato.
- **Modalità offline** — nessuna interfaccia offline specifica o indicatori stato in cache osservati.
- **Profilo/impostazioni** — nessuna schermata profilo oltre selezione lingua; nessun cambio password o impostazioni account.
- **Web o desktop** — analizzata solo interfaccia mobile; altre piattaforme non validate.
