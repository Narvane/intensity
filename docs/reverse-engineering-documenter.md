...
| Versão | Propósito |
|--------|-----------|
| **Curta** | Essência do documento em leitura rápida |
| **Média** | A mesma mensagem, com mais contexto e conexão entre ideias |
...
- Curta, Média e Detalhada diferem em densidade explicativa, não em tipo de conteúdo.
- A versão detalhada **não** autoriza antecipar conteúdo de camadas posteriores.
- Se a versão detalhada introduzir fluxos específicos, parametrizações, classificações, escalas, quantidades ou regras operacionais, houve vazamento de camada.
...
- Conceitos centrais percebidos pelo usuário (ex.: nomes de experiências, caixinhas e terminologias de identidade), sempre em nível conceitual
...
- Regras específicas de funcionamento, parametrizações, classificações internas, escalas numéricas, quantidades, condições e fluxos passo a passo
...
| Conceitos de domínio percebidos pelo usuário | ✓ (conceito) | ✓ (detalhamento funcional) | — | — |
| Regras de negócio e parametrizações | — | ✓ | ✓ (impacto estrutural) | ✓ (implementação) |
...
O Inglês (`en`) deve ser tratado como idioma canônico da documentação.

A análise da aplicação, interpretação dos conceitos e construção inicial do conhecimento devem utilizar o Inglês como referência principal.

As versões em Português Brasileiro e Italiano devem representar fielmente o conteúdo da versão em Inglês.
...
1. Gerar a versão em Inglês.
2. Gerar a versão correspondente em Português Brasileiro.
3. Gerar a versão correspondente em Italiano.

Ao atualizar uma documentação existente:

1. Atualizar a versão em Inglês.
2. Replicar a atualização para Português Brasileiro.
...
Todos os nomes de documentos, diretórios documentais e arquivos Markdown devem utilizar Inglês.
...
As versões traduzidas devem manter exatamente o mesmo nome de arquivo e estrutura de diretórios em todos os idiomas.
...
Organize os arquivos conforme a camada e o documento do mapa utilizando nomes em Inglês (ex.: `product-conception/overview-what-is-it.md`).
...
- Gerar ou atualizar as versões em Inglês, Português e Italiano.
...
| Versão detalhada da Camada 1 listar escala 1-5, tipos fechados ou regras de sorteio | Manter na Camada 1 apenas a ideia conceitual (ex.: progressão de intensidade); mover regras, faixas e operação para Camada 2 |
