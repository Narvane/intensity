...
Este documento é o **artefato operacional** que transforma `/newdocs` em execução incremental de software. Copie **`/newdocs`** e **este arquivo** para o repositório do projeto e use-os como ponto de partida para o agente — não é necessário nenhum outro material de referência.

Combina técnicas de mercado para especificação e planejamento:

| Técnica | Uso neste plano |
|---------|-----------------|
| **Doc-Driven Development** | `/newdocs/en/` é a única fonte de produto e engenharia |
...
| **Contract-First** | OpenAPI derivado de `integrations-and-communication.md` **antes** da implementação |
...
**Stack obrigatória (Camada 4 — `/newdocs/en/engineering-and-operations/`):**
...
Referências: `tools.md`, `technical-decisions.md` (DT-01 … DT-15), `development-process.md`.
...
| Spec em `/newdocs` | Comportamento, arquitetura e engenharia — inclui convite e exclusão de caixinhas |
| Este plano + `/newdocs` | Únicas entradas obrigatórias para iniciar o agente |
| Idioma canônico da spec | `/newdocs/en/`; traduções pt-br e it para alinhamento de produto |
...
1. **Comportamento** → `/newdocs/en/` Camadas 1–2 (`product-conception/`, `solution-specification/`)
2. **Contrato e integração** → `/newdocs/en/solution-architecture/integrations-and-communication.md` (Detailed)
3. **Estrutura e decisões** → `/newdocs/en/solution-architecture/` + `/newdocs/en/engineering-and-operations/`
4. **Contrato versionado no código** → `openapi/openapi.yaml` (criar na F-1 — §2)
...
```
intensity/
├── newdocs/                        # documentação do produto + este plano
├── openapi/
...
```
1. READ     → /newdocs/en/ (docs do slice)
...
| F1-S2 | `experience-and-identity` | Cores intensidade, chips Exact/Up to |
...
- Referência explícita: spec em `/newdocs/en/`, plano em `/newdocs/plano-desenvolvimento-ia.md`
...
**Docs:** `experience-and-identity.md` Intensity colors · `data-model.md` Draw filters

Labels **Exact** / **Up to**; cores verde→vermelho; parâmetros teal/lime/pink
...
**Docs:** `functional-components.md` Flow D, telas 5 e 14 · `experience-and-identity.md` Invite sharing (green accent)
...
### Contexto
Implementação conforme /newdocs e este plano.
Stack: Spring Boot 3.5 + React 19 + Capacitor 7 (ver /newdocs/en/engineering-and-operations/tools.md).

### Documentação obrigatória
- [listar paths /newdocs/en/ + seções]
...
- [x] Autossuficiente com `/newdocs` + este arquivo
- [x] Stack alinhada a `/newdocs` Camada 4
...
*Spec canônica: `/newdocs/en/` (42 documentos). Plano operacional: este arquivo.*
...
2. Copie a pasta **`newdocs/`** inteira (inclui este plano).
3. Abra o agente com o prompt:

```markdown
Leia integralmente:
- /newdocs/plano-desenvolvimento-ia.md
- /newdocs/en/product-conception/how-it-works.md (Medium)
- /newdocs/en/solution-specification/data-model.md (Medium)
- /newdocs/en/solution-specification/functional-components.md (Medium)
- /newdocs/en/solution-architecture/integrations-and-communication.md (Detailed)

Execute o slice **F-1-S1**, depois F-1-S2 … na ordem do §12.
Respeite DoR/DoD deste plano. Spec canônica: /newdocs/en/.
```
